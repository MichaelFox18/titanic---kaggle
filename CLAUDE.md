...
5. **Threshold tuning was a CV mirage here — don't repeat it.** The 0.5 default consistently under-predicted by ~2 percentage points (iter 6 rate 0.361, iter 9 rate 0.359 vs train 0.384). Iter 10's OOF-validated threshold of 0.47 brought test predicted rate to 0.383 — *apparently* perfect calibration — and it **regressed LB 1.4pt** (0.809 → 0.794, gap widened to 0.061). Lesson: driving the aggregate predicted rate to the train rate is NOT an optimization target; matching the global rate ≠ getting more individual rows right. The predicted-rate check is only useful as an *over*-prediction guard (catching iter 4), not as a calibration objective. Iter 9 (plain 5-seed RF, threshold 0.5) is the best submission; leave the threshold at 0.5.
